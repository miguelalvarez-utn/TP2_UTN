...
* \param arrayA Empleados Array de Empleados
* \param arrayB Empleados Array de Empleados
...
*
*/
int Informes_listarPorCriterio(Empleados arrayA[], Empleados arrayB[], int sizeI, int sizeJ, char* criterio)  //Valores de dos arrays. Si es valor repetido se vuelve a imprimir
{
    int retorno=-1;
    int i;
    int j;
    if(arrayA!=NULL && sizeI>=0 && arrayB!=NULL && sizeJ>=0 && criterio!=NULL)
    {
        for(i=0;i<sizeI;i++)                                                                            //Obtengo la posicion de la primer entidad
        {
            ...
            {
                empleados_buscarID(arrayB,sizeJ,arrayA[i].idUnico,&j);                            //Obtengo la posicion de la 2da entidad buscando por el campo en comun
                printf("\nID A: %d\nID B: %d",
...
int Informes_listarCriterioContadorAcumulado(Empleados arrayA[], Empleados arrayB[], int sizeI, int sizeJ)         //cambiar Empleados
{
    int retorno=-1;
    int i;
    int j;
    ...
    if(arrayA!=NULL && sizeI>=0 && arrayB!=NULL && sizeJ>=0)
    {
        for(i=0;i<sizeI;i++)
        {
            empleados_buscarString(arrayA,i,arrayA[i].nombre,&auxPosicion);                  //cambiar nombreFuncion y campo      va a analizar hasta <i
            ...
            else
            {
                ...
                for(k=i,contador=0,acumulado=0;k<sizeI;k++)                                                            //Recorro por segunda vez el mismo array
                {
                    ...
                    {
                        empleados_buscarID(arrayB,sizeJ,arrayA[k].idUnico,&j);                 //cambiar Empleados, busco por el campo en comun
...
*
*/
int Informes_maxContadorAcumulado(Empleados arrayA[], Empleados arrayB[], int sizeI, int sizeJ)
{
    int retorno=-1;
    int i;
    int j;
    ...
    if(arrayA!=NULL && sizeI>=0 && arrayB!=NULL && sizeJ>=0)
    {
        for(i=0;i<sizeI;i++)
        {
            empleados_buscarString(arrayA,i,arrayA[i].nombre,&auxPosicion);                  //cambiar nombreFuncion y campo
            if(arrayA[i].isEmpty==1 && auxPosicion!=-1)
                continue;                                                                 //Si ese valor ya aparecio > continue
            else
            {
...
*
*/
int Informes_listarAuxiliarOrdenar(Empleados arrayA[], Empleados arrayB[], int sizeI, int sizeJ)         //cambiar Empleados
{
    int retorno=-1;
    int i;
    int j;
    ...
    Empleados arrayAux[sizeI];                                                           //cambiar Empleados y size si corresponde
